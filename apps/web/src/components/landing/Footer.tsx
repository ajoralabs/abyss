				...
				<div className="flex flex-col lg:flex-row justify-between items-start gap-20 mb-32">
					<div className="max-w-xl">
						...
						<div className="flex gap-4">
							<a
								...
								className="group flex items-center gap-2 px-6 py-3 rounded-full border border-white/10 hover:bg-white hover:text-black transition-all duration-300"
							>
								<Github size={20} />
							...
							<a
								href="/discord"
								className="group flex items-center gap-2 px-6 py-3 rounded-full border border-white/10 hover:border-discord hover:text-discord transition-all duration-300 hover:bg-[#5865F2]/10"
							>
					...
					<div className="grid grid-cols-2 gap-20">
									...
									(item) => (
										<li key={item}>
											<a
												...
												className="text-xl text-gray-300 hover:text-neon-cyan transition-colors flex items-center gap-2 group"
											>
												{item}
												<ArrowUpRight
													size={16}
						...
						<div>
							...
							<ul className="space-y-4">
								{['Privacy Policy', 'Terms of Service', 'License'].map(
									(item) => (
										<li key={item}>
											<a
												href={`/${item.toLowerCase().replace(' ', '-')}`}
												className="text-xl text-gray-300 hover:text-neon-cyan transition-colors"
											>
												{item}
											</a>
										</li>
